"""DeepInfra engine — OpenAI-compatible API with streaming and image support."""
import base64
import json
...
class DeepInfraEngine:
    """Inference via DeepInfra OpenAI-compatible API."""
    ...
    async def load(self):
        ...
        print(f"DeepInfra initialized: model={self.model}")
    ...
    async def generate(
        ...
        """Generate a non-streaming response from DeepInfra.
        ...
        if resp.status_code != 200:
            ...
            raise RuntimeError(f"DeepInfra API error ({resp.status_code}): {error_msg}")
        data = resp.json()
    ...
    async def generate_stream(
        ...
        """Stream tokens from DeepInfra via SSE.
        ...
        async with self._client.stream(
            ...
            if resp.status_code != 200:
                ...
                raise RuntimeError(f"DeepInfra streaming error ({resp.status_code}): {body.decode()}")
